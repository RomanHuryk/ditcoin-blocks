...
{
// declare struct in ditcoin's cryptonote namespace.
// ditcoin should provide definition for this,
...
public:
    ...
    rpccalls(string _deamon_url = "http:://127.0.0.1:19096",
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Ditcoin deamon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Ditcoin deamon at "
                 << deamon_url << endl;
            return false;
        }
