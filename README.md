# Onion Ditcoin Blockchain Explorer

Currently available Ditcoin blockchain explorers have several limitations which are of 
special importance to privacy-oriented users:
 ...
 - do not support Ditcoin testnet,
 - have limited JSON API.


In this example, these limitations are addressed by development of
an Onion Ditcoin Blockchain Explorer. The example not only shows how to use 
Ditcoin C++ libraries, but also demonstrates how to use:
...
Clearnet mainet Ditcoin version:

 - 
  
Clearnet testnet Ditcoin version:

 - 


## Onion Ditcoin Blockchain Explorer features

The key features of the Onion Ditcoin Blockchain Explorer are:
...
git clone https://github.com/ditcoin/ditcoin-core
...
Once the Ditcoin is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/ditcoin-core
cd ~

# download the source code
git clone https://github.com/ditcoin/ditcoin-explorer.git
...
# altearnatively can use: cmake -DMONERO_DIR=/path/to/ditcoin_folder .. 
# if ditcoin is not in ~/ditcoin-core

# compile
make
```

When compilation finishes executable `ditcoin-blocks` should be created. Before running
...
By default it will look for blockchain in its default location i.e., `~/.ditcoin/lmdb`.
You can use `--bc-path` option if its in different location. 
Example output:

```bash
[mwo@arch ditcoin-explorer]$ ./ditcoin-blocks
...
```
ditcoin-blocks, Onion Ditcoin Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Ditcoin total emission monitoring
                                        thread
  ...
  --ssl-key-file arg                    path to key file for ssl (https) 
                                        functionality
  -d [ --deamon-url ] arg (=http:://127.0.0.1:19096)
                                        Ditcoin deamon url
```
...
alias ditcoin-blocks-mainnet='~/ditcoin-explorer/build/ditcoin-blocks    --port 8081 --no-blocks-on-index 24 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias ditcoin-blocks-testnet='~/ditcoin-explorer/build/ditcoin-blocks -t --port 8082 --no-blocks-on-index 24 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Ditcoin emission 

Obtaining current Ditcoin emission amount is not straight forward. Thus, by default it is 
disabled. To enable it use `--enable-emission-monitor` flag, e.g., 


```bash
ditcoin-blocks --enable-emission-monitor 
```
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.ditcoin/lmdb/emission_amount.txt`. For testnet network, 
 it is `~/.ditcoin/testnet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.ditcoin/lmdb/emission_amount.txt`
...
```
Ditcoin emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Ditcoin daemon's  `print_coinbase_tx_sum` command. 
...
Having the `crt` and `key` files, run `ditcoin-blocks` in the following way:

```bash
./ditcoin-blocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
...
```bash
# we use here official Ditcoin project's donation address as an example
...
Return raw json block data, as represented in Ditcoin.
...
Return raw json tx data, as represented in Ditcoin.
